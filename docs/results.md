...
## 榜单 · smoke（24 条，预生成 3 条，显示前 6 名与我们）

| # | 真实# | 队伍 | 通过率 | 功能率 | 费用 | 耗时 | 级别 |
|---|---|---|---|---|---|---|---|
| 1 | 1 | VOLO AI | 100% | 100% | ¥0.0001 | 0s | senior |
| 2 | 2 | 你也秃对不队 | 100% | 100% | ¥0.0031 | 2s | senior |
| 3 | 3 | Orchestra | 100% | 100% | ¥0.0037 | 4s | senior |
| 4 | 4 | 李尧 | 100% | 100% | ¥0.0051 | 23s | senior |
| 5 | 5 | octos | 100% | 100% | ¥0.0052 | 22s | senior |
| 6 | 6 | niceeeeee | 100% | 100% | ¥0.02 | 10s | senior |

## 榜单 · smoke-evolution（14 条，预生成 2 条，显示前 6 名与我们）

| # | 真实# | 队伍 | 通过率 | 功能率 | 费用 | 耗时 | 级别 |
|---|---|---|---|---|---|---|---|
| 1 | 1 | 你也秃对不队 | 100% | 100% | ¥0.0000 | 2s | senior |
| 2 | 2 | VOLO AI | 100% | 100% | ¥0.0001 | 1s | senior |
| 3 | 3 | Orchestra | 100% | 100% | ¥0.0046 | 4s | senior |
| 4 | 4 | octos | 100% | 100% | ¥0.0055 | 36s | senior |
| 5 | 5 | 李尧 | 100% | 100% | ¥0.0086 | 30s | senior |
| 6 | 6 | niceeeeee | 100% | 100% | ¥0.02 | 8s | senior |

## 榜单 · ticket-booking（32 条，预生成 3 条，显示前 6 名与我们）

| # | 真实# | 队伍 | 通过率 | 功能率 | 费用 | 耗时 | 级别 |
|---|---|---|---|---|---|---|---|
| 1 | 1 | 你也秃对不队 | 80% | 0% | ¥0.0000 | 2s | senior |
| 2 | 2 | octos | 90% | 50% | ¥0.13 | 100s | senior |
| 3 | 3 | 李尧 | 90% | 50% | ¥0.25 | 3m18s | senior |
| 4 | 4 | net1111 | 100% | 100% | ¥0.42 | 4m17s | senior |
| 5 | 5 | Orchestra | 100% | 100% | ¥0.61 | 10m39s | senior |
| 6 | 6 | VOLO AI | 100% | 100% | ¥0.63 | 4m39s | senior |
...
最后更新：2026-09-14 13:20 UTC（数据来自实时榜单；「达成」只认云端数字且需连续两次）

| 赛道 | 我们（最好账号） | 费用 >0 且满分的对手（前 3，按费用） | 目标 | 差距（对最便宜对手） | 状态 |
|---|---|---|---|---|---|
| Smoke | 李尧 100% ¥0.0051 / 23s | VOLO AI ¥0.0001/0s；你也秃对不队 ¥0.0031/2s；Orchestra ¥0.0037/4s | 两题合计 ≤ ¥0.0028 | ¥+0.0050 | 未达成 |
| Smoke Evolution | octos 100% ¥0.0055 / 36s | 你也秃对不队 ¥0.0000/2s；VOLO AI ¥0.0001/1s；Orchestra ¥0.0046/4s | 两题合计 ≤ ¥0.0040 | ¥+0.0054 | 未达成 |
| Ticket Booking | octos 90% ¥0.1344 / 100s | net1111 ¥0.4235/257s；Orchestra ¥0.6059/639s；VOLO AI ¥0.6297/279s | 10/10 稳定且 ≤ ¥0.15（连续两次） | 通过率 90% 未满分（满分最低对手 ¥0.4235） | 未达成 |
| ARC-Bench Web | 未上榜 | 无满分对手 | 六题完成即第一，随后压总费用 | — | 未上榜（六题完成 2/6） |

- 口径说明：§0 表把 Smoke 的「需要超过的条目」写为 ¥0.0031（你也秃对不队）、Evolution 为 ¥0.0046（Orchestra），即把 VOLO AI ¥0.0001 / 0 s 与你也秃对不队 ¥0.00005 / 2 s 视作预置；按统筹给的「费用 = 0 才算预置」规则，这两条费用 > 0，严格口径下 Smoke / Evolution 的最便宜对手是它们（¥0.0001、¥0.00005），任何调用模型的运行都无法低于。建议统筹确认：是否把「费用 < ¥0.001 且耗时 < 5 s」也归为预置。表中「差距」按严格口径计算，目标列按 §0 原文。

## 窗口（ARCBENCH key 占用状态 · 所有工作流跑本机/云端前先看这里）

最后更新：2026-09-14 13:20 UTC（工作流 C 维护；有云端运行时 key 必须空闲，本机运行会污染平台计费）
...
| 下一空窗计划（A 已合入 #81/#94/#95/#96，包 main@b96f345f） | 固定顺序：Smoke ×2 账号 → octos Evolution → TB ×2（两账号各一次）。A 离线估算：counter ≈¥0.0011–0.0012、dice ≈¥0.0009（极小档位跳过探测、只回页面片段）；TB 全套修复轮交付最优轮；Web 六题第二遍用新包（N 节点单请求 codegen，预期每节点 36 → 1–3 请求）等队列结束后排 | 紧随 stackoverflow |
