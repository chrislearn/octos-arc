...
def main():
    ...
    env = dict(os.environ, OCTOS_BIN=str(args.binary.resolve()), OCTOS_TIME_BUDGET=str(args.seconds),
               ...
               OCTOS_ARC_REASONING='none', OCTOS_ARC_IMPLEMENT_REASONING='none',
    ...
    manifest = {'models': args.models, 'budget_seconds': args.seconds, 'thinking': False,
    ...
    for index, model in enumerate(args.models):
        ...
        folder.mkdir()
        print(f'Start {model}, fresh Keep, budget {args.seconds}s', flush=True)
        run = bounded([sys.executable, str(args.adapter / 'run-task-local.py'), str(args.task),
                       ...
                       '--port', str(args.port + index * 4)], folder / 'run.log', env, args.seconds + 300)
