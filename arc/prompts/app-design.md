Installed helper interfaces are fixed: backend/lib/store exports read, write, update, migrate, onReset, reset; backend/lib/collection exports collection. Frontend shared/request.js exports requestJson (raw JSON). Reuse these exact APIs; do not invent load/save aliases or plan replacement helpers. migrations is an array of objects with a unique nonempty string id and a synchronous up(data) function, not an array of functions.
