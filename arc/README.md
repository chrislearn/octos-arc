...
顺序实现遇到构建/启动故障时，先做有界局部恢复；必要时进入一次受现有最终修复轮数限制的全套启动恢复，获得完整测量后继续剩余需求，不要求未实现的功能先通过。局部修复若破坏构建，即使当前需求原来为 0/N，也恢复此前可构建的源码快照并复测；复测不完整不会视为恢复成功。恢复和继续实现仍遵守全局预算与最终阶段保留时间。
...
明确的认证失败、余额不足或 `insufficient_quota` 会在本次运行的代理内按服务端点和账户凭据锁定；后续本地重试不再发送上游请求，也不重复记为上游用量。普通 429 限流与模型权限错误仍保留原有处理。已经发出的并发请求无法撤回。

顺序实现遇到账户中断且没有完整交付检查点时，先将当前源码保存在 Git，再在剩余时间允许时运行全量验收，不再调用模型。若当前产物不可用或没有任何通过项，则尝试较早的健康检查点并重新全量验证；失败时恢复原始中断源码。只有完整测量且存在通过项才交付评测，仍记录生成中断事件；部分回归成绩不会被升级为全量结果。已有完整交付检查点继续使用原有恢复路径。

### 检查点覆盖与诊断

检查点除已验证功能外，默认轮流复测最多四项历史失败功能（`OCTOS_ARC_CHECKPOINT_BACKLOG=4`，0 关闭）。未开始的功能不加入该队列；日志和 `checkpoint_coverage` 指标分别记录本次覆盖、尚未观察及待复测数量。报告缺项、加载失败或中断均不能证明恢复。完整覆盖的检查点可保存为交付检查点。

检查点修复默认每轮最多聚焦四个失败节点（`OCTOS_ARC_CHECKPOINT_REPAIR_NODES=4`）；后续轮次优先处理尚未尝试的节点。每次有效修改仍复测整个检查点，不增加既有修复轮数和请求上限。初次实现修改源码也会根据依赖关系检查已验证的相关行为，因此可能增加本地验证时间；仍受剩余预算和最终测量预留约束。

导入路径和重复路由提示属于源码启发式线索，不替代编译器，不拒绝合法中间件或自定义解析器。启动日志优先显示编译位置与具体原因；连接失败不直接推断为路由异常。每次运行记录适配层 Python 源码与蓝图的 SHA-256（`adapter_provenance`），不包含环境变量和凭据；该指纹不覆盖外部模型、运行时依赖或平台计费状态。

### 流式输出与超时续期

无工具的 implement/repair 流式路径共享代理和 stdio 外层的进度截止时间：只有非空正文或 reasoning_content 更新进度，空白、usage 和心跳不续期。原回合期限附近仍有输出时，可等待至最后有效输出后 120 秒，但额外时间不超过原预算的 25% 或 120 秒，且受任务剩余时间扣除最终验收预留的限制。工具调用和非流式路径保持原预算。

流式请求收到响应头后，首次有效输出最多等 300 秒；首次输出后停滞 120 秒会停止。原回合截止时间/续期上限更早时以其为准。读取正文期间的 watchdog 可中断静默 socket，心跳不能规避停滞检测；正文和推理均检查强重复。超时的持久 stdio 会话会关闭，迟到请求不能给后续回合续时。中断结果保留部分内容并标为 length，不伪装成功。

范围：本功能控制 Python 适配层。代理仍汇总完成后返回给内核，所用外部内核的 HTTP/首 token/整体超时仍可能更早触发；没有修改或重新发布内核。尚未以线上模型评测验证耗时和得分变化。v7.8.zip 是此前已复核快照，本次源码修改未写入该包。
