...
Integration audit: a declared feature component, owner state or callback is unfinished until it is mounted and connected in every applicable rendering branch. Trace each required gesture through the visible control, handler, request/state transition and rendered result. A successful build does not prove styles are active; utility classes require their local compiler/import, while ordinary classes require bundled definitions.
...
Inactive repeated-item toolbars must not expose clickable invisible controls: opacity:0 alone is not hidden. Use visibility:hidden for inactive actions, reveal on hover/focus-within or explicit activation, and keep the owning record ID stable while its menu/dialog is open. Verify an action on a non-first record leaves the first unchanged. A menu's onOpenChange(false) must not close its sibling dialog. Render that dialog outside conditional menu content so menu unmount cannot destroy it. Nested option dialogs update the editor's draft; their Done must not ambiguously replace the parent editor's commit/completion control.
