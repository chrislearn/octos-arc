...
- 应用设计及交互契约明确区分固定的内建记录与可编辑记录，要求为嵌套菜单/对话框分配 Escape、外部点击和焦点转换的所有权；修复提示在请求额度内优先落地可验证源码改动。静态检查对 `onOpenChange` 直接提交保存给出提示性告警，需通过实际交互确认，不判定为必然错误。
- Python 3.12 全套 797 项通过（19 项跳过）；`git diff --check` 与语法检查通过。未执行新的线上模型评测。

## 2026-09-23：通用 React 与数据响应契约检查

- 分批源码检查增加事务回调仅返回过滤数组、空所有者与按所有者读取冲突、React 空处理器、异步 `useEffect`、嵌套路由缺少 `Outlet`、以及把 `requestJson` 当 `Response`/成功信封使用的提示。均为限量、增量、提示性诊断；自定义请求/集合适配器不套用原版返回契约。
- `requestJson` 保持成功时直接返回 JSON、HTTP 失败抛出带 `status` 的异常。空成功响应返回 `null`，无效 JSON 也抛出带 `status` 的明确异常；普通对象请求体自动序列化，FormData/URLSearchParams 保留原生编码。React 提示词只在两个共享适配器均未修改时声明其返回契约。
- 本地 API 配置允许可选 `visual_model` 字段，使用 `.test_data` 的 smoke counter 生成及验收 1/1 通过。Python 3.12 全套 794 项通过（19 项跳过）；静态检查在下载的 Ctrip 产物中提示了三处已确认的流程缺陷。未修改任务定义或公开测试。
- 下载产物的 Ctrip 125 项公开测试在隔离副本中本地运行，前后端构建通过，93/125 通过。选取订单中心、联系人新增和专票配置三项失败以单 worker 复测，均重现；定位到可见入口/表单交互链，而非并发造成的偶发失败。此分数是旧产物基线，不代表本轮适配器改动后的线上成绩。

## 流式进度续期（v7.8 打包后，工作区）

- 无工具 implement/repair 的代理与 stdio 共享有上限的进度续期：最多原回合预算 25% / 120 秒，并保留最终验收时间。
- 有效正文/推理才续期；首次输出、停滞、硬截止和强重复分别约束。读取中 watchdog 主动中断 socket，部分结果标为 length。
- 关闭超时持久会话和已结束的续期对象，避免迟到输出续期后续回合。pack.sh 纳入新模块。
- 外部内核仍有独立超时限制，未改内核版本；v7.8.zip 不变。
- 验证：全套 788 项，769 通过、19 跳过（32.293 秒）；包含真实 HTTP 持续/静默流、外层续时、验收预留和超时会话清理测试。未执行线上模型评测。

## v7.8 打包复核

复核补齐：检查点修复后的报告缺项、加载失败或中断时，将本轮相关 verdict 标为未知并留待复测，不再根据不完整报告的表面分数触发严重回归回滚。历史失败证据仍保留供后续修复使用。增加对应回归测试并补齐既有测试的完整结果夹具。

最终验证：Python 3.12.14、Node 24.19.0，全套 779 项，760 通过、19 跳过（31.367 秒），差异检查通过。按 `pack.sh` 打包，107 个 ZIP 条目通过 CRC、重复路径、与工作区源码逐文件一致性检查；解压后 `main.py --help` 通过。未自带 Linux 内核，使用既有发布下载配置。未执行线上模型评测。

## 2026-09-22：回归覆盖、分批修复与诊断证据

- 初次实现前保存源码版本，验收通过后也检查受改动影响的已验证行为；相关复测失败或不完整时更新旧 verdict，避免仍保留过时的通过状态。
- 检查点默认轮流纳入四项此前失败功能，记录本轮覆盖和未观察数量，不扩展到尚未开始的需求。轮询失败项不会永久膨胀回归集合；完整检查点可保存交付快照。不完整的检查点/修复报告不能证明恢复。
- 广泛检查点失败默认每轮聚焦最多四个节点，后续轮次优先处理未尝试项；有效修改后仍验证整个检查点，保持原有轮数、请求上限和最终阶段预留。
- 增加导入路径、跨文件重复路由的提示性检查，明确允许解析器插件、不同挂载前缀和合法中间件链，不据此直接拒绝代码。
- 构建日志使用错误摘要而非前 300 字符，保留编译位置、导入/导出错误。传输失败与进程退出不再直接归因为静态路由异常。运行日志和指标新增适配层 Python 代码及蓝图源码指纹，不含环境变量和凭据。
- 新增九项任务无关测试；更新既有 mock 签名和不完整结果夹具，使旧测试继续验证原本意图。

验证：Python 3.12.14、Node 24.19.0，全套 778 项，759 通过、19 跳过（31.819 秒）；`git diff --check` 通过。未改 tasks 或公开验收测试，未执行付费模型 benchmark。更早回归检查会增加部分本地验证成本，线上收益仍需对照测量。

## 2026-09-22：额度熔断与顺序实现中断交付

- 代理按服务端点、认证凭据及账户范围锁定明确的认证/余额/额度错误，跨输入、模型和阶段阻止重复上游请求；普通限流和模型权限错误不触发该锁定。被阻止的本地重试单独计数，不重复计入上游用量；已经发出的请求不受追溯取消。
- 完善 `insufficient_quota` 识别。没有完整交付检查点时，先保存中断源码，再在剩余时间允许时执行无模型调用的全量验收。当前版本不可用或零通过时，可恢复健康检查点重新测量；未形成可交付结果则恢复原始源码。部分检查点成绩不会代替全量结果，生成中断事件仍保留。
- 新增六项通用测试，覆盖账户隔离、限流分类、完整测量、健康版本回退、失败回退和预算/源码保存限制。

验证：Python 3.12.14、Node 24.19.0，全套 769 项，750 通过、19 跳过（32.129 秒）；`git diff --check` 通过。未修改 tasks、公开测试或任务特定提示词，未调用付费模型重跑 benchmark，线上分数和成本效果尚待验证。

## 2026-09-22：通用恢复队列、构建回退与有界无改动重试

- 顺序实现的局部启动恢复失败后，在预算允许时使用现有全套修复机制恢复基础设施；首次获得完整测量即回到原需求队列，不再把尚未实现的功能留给最终修复兜底。恢复仍受既有修复轮数、全局时间/成本守卫和最终阶段准入条件约束；测量不完整不放行。
- 当前需求即使只有 0/N，通过测量确认过的可构建源码也是有效回退点。后续修复破坏构建时恢复该快照并重新测量；没有复测预算或复测仍不完整时返回未知状态，不宣称恢复成功。
- 最终修复单次无有效源码变化后允许一次有界策略切换。源码摘要匹配时复用已有失败报告，跳过重复测量；源码变化或恢复了不同快照则重新测量。连续无效尝试仍停止，默认三个周期及显式周期上限保留，已有提升不会因一次无效尝试而立即丢失。
- 新增 14 项通用回归，覆盖队列恢复、不完整测量、预算边界、零通过回退、无改动重试和缓存失效。包括真实 Node 模块导入失败与 Git 工作区恢复测试，验证脏文件/新增文件被撤销后重新运行成功。更新一项原有停止条件测试及 README 中过时的周期上限说明。

验证：Python 3.12.14、Node 24.19.0，全套 763 项，744 通过、19 项可选测试跳过（30.809 秒）；`git diff --check` 通过。系统 Node 26 的首次全套运行有一项既有 reporter 测试触发 10 秒超时，切换到上述 runtime 后该测试及全套通过。没有修改任务需求、公开验收测试、应用蓝图或任务特定提示词；本轮没有调用模型重跑 benchmark，因此不据此声称通过率或成本已改善。

## v5 后续：大型前端模块与无效样式检查（待云端 A/B）

复盘线上运行 `d372c456e1fa`：最终 27/32，但生成的 `App.jsx` 达 37.5KB/1026 行；标签选择组件和 owner state 已声明却没有挂载/传递到两个列表分支。页面同时使用 109 个 Tailwind 风格类名，manifest 和 CSS 却没有 Tailwind 编译链，构建成功但颜色等视觉状态实际不变。这些是跨任务的模块化、接线与构建完整性问题，不是便签业务或 CRUD 问题。

应用级设计 schema 新增稳定 `modules` 所有权表并升级缓存版本；React 固定契约把 `App.jsx` 限定为路由/组合入口，应用模块以 18KB 为软上限，超过前按页面、布局、复用编辑器/列表和 API/state concern 拆分，同时明确禁止为凑数字制造透传文件。每个节点继续收到同一模块表，目的是降低后续修复时重复携带/读取单个巨型文件的输入 token，而不是保证一次生成更少代码。

通用 scaffold 构建前新增保守的 utility CSS 检查：只有源码出现至少 10 个不同的 utility 风格 token、又既无 Tailwind 依赖/import、也没有为大部分 token 提供显式 CSS 选择器时才报错。这样能在昂贵的浏览器视觉验收前暴露“类名存在但样式不生效”；少量相似命名、完整手写 utility sheet 和正确的本地 Tailwind 构建均放行。React 契约另要求从可见 gesture 追到 mounted control、handler、持久化/state 和 rendered result，防止只声明未接线。全套 Python 回归 687 项通过、18 项可选测试跳过；真实 token、缓存命中率和通过率仍需同题云端 A/B。

同一运行并不是在 27/32 达到某个分数阈值后主动结束，也没有耗完时间：32 节点总预算是 48,000 秒，实际生成约 57 分钟，成本守卫也未触发。真正的提前结束条件是最终修复的局部请求达到 8 次请求上限且未写入源码后，`final_repair_no_change` 直接终止了所有外层全量修复周期。现在单次无写入只结束本轮并强制下一周期切换到定向工具策略；整轮无提升也在预算充足时换方法继续。只有全绿、显式成本守卫或剩余时间不足以完成“测量→修复→复测”才提前结束。大型任务另从既有总预算内保留最终阶段容量（默认通常 600 秒、不超过阶段预算 25%），逐叶与 checkpoint 修复不得借用。该调度完全基于测试状态和预算，不识别具体业务。

## v5 后续：跨任务复核修复（尚未云端评测）

验收默认并发按现有平台日志改为 1，预期平台并发与内部压力测试配置分离；Python/Rust 默认值一致。FILE/EDIT 相同内容不再算有效写入。规划保留场景规则，预算不足先保全结构目录再均摊详情；11 棵任务树均验证 12k 字符下不丢需求 ID。设计校验方法/路径/重复项/契约结构，稳定前缀与节点片段共用一个预算，不再截断 JSON；缓存身份基于完整需求树，不是截断后的摘要。

依赖推荐读取场景步骤、过滤明确否定项，Markdown 不再全局排斥独立富文本编辑器。增加可选、带同文件执行账本的同步迁移接口；保留用户编辑与删除，不自动重灌种子。设计契约补充状态归属、原子命令/撤销、时间与控件语义，不预置题目业务。生成波次优先同父边界，缩批后记住上限，格式重试共用截止时间；Vite 相对错误路径可直接引用源码。首次套件后仅对跨需求重复的明确运行错误尝试一次共同修复，真实改动才重跑完整套件，新回归/不可靠结果则回退。回退修复了新提交文件残留问题。完整说明见 `dev-docs/task-audit-fixes.md`。

## v5 后续：固定 Web 框架基线（未进行新一轮云端评测）

新建且至少 3 叶的 codegen 应用在规划前安装 React/Vite/Radix/React Router 基线，直接版本和核心传递依赖 lockfile 固定；小任务与 evolution 保留原架构。审计全部 11 棵任务树后增加按通用能力筛选的精确版本推荐（表单、日期、Markdown、富文本、金额、轮播、图标、PDF、Tailwind），不预装全部库、不内置题目数据或测试答案。React 状态与 Radix 交互约束进入规划/生成/修复的固定前缀。

补齐 JSX/TSX/Vue 源码引用和快照，锁文件退出模型上下文但参与安装缓存；修复 Vite 项目级 public 资源、React Router 深层刷新检查和框架源码 CDN 检查。取消“关闭编辑器一律保存”的假设。全套 codegen 修复验证实际应用结果，格式纠正最多一次、共享原轮次截止时间，失败转工具修复。细节与验证方式见 `dev-docs/web-stack-baseline.md`；未重编发布内核、未覆盖 v5.1 压缩包。

## v5 运行修复（待同题云端验证）

运行 `6c108b649a50` 的 32 叶整应用请求输入约 62K 字符、但没有产生文件块；后续波次对结束标记或同文件混用 FILE/EDIT 的回复反复折半。默认把整应用单轮限制为最多 12 叶，大任务直接按波次生成；对独立成行的 `<END EDIT>` / `END EDIT` 等结束标记做保守解析，对真正冲突的格式在同批次只重发一次，再决定是否缩批。日志分别记录成功波次、组尝试和实际模型轮次。部分波次已写入时不再因最后一叶失败而重做全部节点；未触达的叶子不会误标为已实现。

本次真正挡住首次全套测试的是静态路径被同方法参数路径遮蔽。架构和生成提示加入跨任务的具体顺序例子，同时保留代码层的路由检查；生成后先对具体启动错误引用的文件做有界定点修复并重测。首次套件仍无法给出可靠结果时，保留已生成应用，交给最终套件修复，不再退回 32 叶重新实现。Python 单元回归覆盖格式、路由修复、部分波次和回退策略；通过率及节省量仍需非并行云端 A/B。

审计了疑似历史 hash：`prompt_sha256` 只写在代理用量日志用于缓存前缀分析，`tree_sha256` / `design_sha256` 只用于本地设计复用校验；源码注释中的历史运行 ID 不会自动进入 Python 生成提示，因此移除它们不会节省模型 token。octos 的工具自由生成轮次禁用工具，sandbox 不影响这些回复；工具修复轮次使用容器内 `danger-full-access` 以允许 npm/测试，外层仍是平台 Docker，受保护目录另有写入钩子与恢复。没有证据表明 sandbox 导致路由遮蔽或文件块格式错误，因此这版未改 Rust 沙箱或重编二进制。

## v5：整应用生成实验（待云端评测）

全新、多节点、具备完整本地验收映射的任务默认先走 `OCTOS_ARC_WHOLE_APP=1` 实验路径：先生成一次全树应用设计，整树提示装得下时用一次 codegen 请求生成应用；否则按依赖顺序分成多节点生成波次，默认每波最多 16 叶，同时受 spec/需求详情/实际提示词预算约束。整树回复若截断也改走波次；单波装不下或截断则折半，直到一叶仍失败才退回原逐节点路径。即使全树设计回复无效，波次仍携带有界的全树需求摘要。生成后以 grader-like 启动方式跑完整测试套件；通过的叶子不再产生单独生成轮次，仅失败叶子进入现有逐节点验收与定点修复，最后继续运行原有全套复测、回退和启动彩排。未改变 evolution 或无测试任务。`OCTOS_ARC_WHOLE_APP=0` 完全退回 v4.3 路径；首轮全套测试无可靠结果或失败项无法映射时也自动回退。单轮上限 `OCTOS_ARC_WHOLE_APP_TIMEOUT` 默认 1800 秒；每波上限 `OCTOS_ARC_WHOLE_APP_WAVE_NODES` 默认 16。

对六个 Web 任务检查了实际需求树、公开 spec 和通用框架：Keep、BookStack 整树提示各约 58K 字符，可尝试一轮；12306、Ctrip、PrestaShop、Stack Overflow 的整树提示超预算，按最多 16 叶的波次分别规划为 8、8、6、5 轮（在尚未生成业务源码的本地静态检查中）；每轮实际会重新预算，后续源码变大时继续缩批或退回工具流程。是否能达到高通过率、节省多少 token 和时间，仍需同题云端测试；不把本地提示词可装入误写为通过率结论。

## v4.3：减少无效生成轮次、约束异步 UI 生命周期（待云端评测）

v4.2 Keep 最终为 31/32：Update Note 在打开编辑器前等待标签请求，背景搜索框仍可见，后续输入可能命中背景；关闭编辑器时又在保存及列表刷新完成前移除界面。本版在应用设计、逐节点 codegen、工具模式 UI 契约中加入通用状态转移约束：编辑器/弹窗控件及背景屏蔽在首次 `await` 前同步就绪，保存后等待持久化及刷新，失败保留编辑内容。不预置便签字段或测试专用文案。

v4.2 对比 v4：模型请求 413 vs 345、工具调用 349 vs 308；4 次生成输出截断、5 次精确编辑锚点失败。此版允许 codegen 精确回复 `<<<NO CHANGE>>>`，但仍逐叶子验收；截断后先测现有应用，未通过才做一次小型 EDIT 重试，再回退工具；修复提示统一使用局部 EDIT，避免反复输出完整大文件。对同父相邻叶子继续保持最多 3 个一批，新增每批首轮通过率日志；v2/v3/v4/v4.2 的已成功批次首轮通过分别为 8/10、5/12、7/10、2/6，差异不足以证明批处理本身造成错误，因此暂不改阈值。完整 Python 本地回归通过；token 与通过率须同题云端测量。
...
- 已选源码按后端入口 → 历史低改动频率 → 路径排序；仅调整提示词呈现，不改变相关性选择、预算或未引用文件写保护。没有 git 历史时退回路径序。日志归因将批次请求单列为 `_batch`，避免重复计费到三个叶子。
- v1 的连续全绿复测、严重 codegen 回归回退和 usage 对账保留。修复提示词本就按静态规则 → 源码 → 失败证据布局；tiny 层已由 spec 大小条件启用，大型 Web 题的公开 spec 不落入 tiny 层，因此本版没有为这两项引入新的风险性重排。
- 本地完整 Python 测试通过；真实 token、费用和通过率需同题云端对照。此版不含通用或题目专用业务模板。
...
## 2026-09-19：Keep 新日志后的修复效率改进

依据 `temp-logs/1/keep.txt`（41333ec2473d）与同题旧日志 `temp-logs/0/1 copy 7.txt`
（ce54f9a57bdc）：最终验收 31/32 → 32/32，代理总 tokens 29,613,391 → 6,829,761，
请求 615 → 217。旧平台计量与代理差约 23M tokens，且同 key 的旧任务时间重叠，费用差不能全部归因于本题优化。
这些是**此前那轮改动的实测**，不是下面这次修改的收益。

本轮改动以 Python 编排路径为主：

- 节点修复被写保护拒绝时，先完整引用被拒文件并重试一次；仍不能应用时在同一个修复时间预算内转工具。
  不再立刻重测原代码并以“相同失败两次”消耗修复轮。记录每次生成自己的 refused/written 集合，
  同一路径重复被拒也可识别；部分文件已经写入时，从当前磁盘重建提示词。时间或全局用量耗尽则停止继续请求。
- 修复提示词也使用按相关性排序的完整源码，优先被拒文件。源码选取新增通用的字面量 `page.goto(...)`
  路径映射，优先级在已知被拒文件之后、文件名猜测之前；保留预算和未完整引用文件的写保护。
  动态 URL 不推断，不添加 Keep 特例。
- 检查默认/显式 `node <entry.js|cjs|mjs>` 的入口是否缺失，在生成提示词中明确要求本轮补齐。
  不改变 `has_app()` 的工程识别语义，不丢弃部分已有页面；不要求框架启动命令额外创建 server.js。
- 输出截断后，工具恢复改为检查当前文件并做针对性的局部编辑，不再固定从 backend/server.js 开始，
  也不声称之前的工具调用都没落盘。恢复提示同步到 Rust 共用模板；剩余节点时间为零时不再重试。

验证：新增 `tests/test_keep_efficiency.py` 14 项，覆盖拒绝后的重试/工具兜底/验收顺序、部分写入后刷新源码、
重复拒绝、时间及费用守卫、启动入口、截断后的节点流程与模板一致性。
用 Keep 的真实 spec/helper 配合 60k 首页与多个约 18k 次级页面夹具，验证 6 个受影响节点均完整引用首页；
这只是提示词选取验证，不是模型生成或真实 Keep 验收。
完整命令 `python3 -m unittest discover -s arc/tests -t arc`：415 项，407 通过、8 跳过；`git diff --check` 通过。

本轮**未调用真实模型、未重跑云端评测**，新增优化的 tokens、费用及通过率变化仍待下一份日志验证。
尚未引入 codegen 局部补丁协议或重构生成应用的前端模块。
